/**
 * pngu-auth — Decap CMS GitHub OAuth backend (plain-JS copy).
 *
 * This file exists for the Cloudflare dashboard's quick-edit mode, which
 * only accepts JavaScript. The canonical source is index.ts in the same
 * directory — keep them in sync if you change one.
 *
 ...
 *
 * Env vars (set in dashboard → Settings → Variables and Secrets):
 *   GITHUB_CLIENT_ID      — public OAuth app client id (Plaintext)
 *   GITHUB_CLIENT_SECRET  — secret (Secret)
 *   ALLOWED_ORIGINS       — comma-separated allow-list of origins permitted
 *                           to receive the token via postMessage
...
async function handleCallback(request, env, url) {
  ...
  try {
    ...
    return adminPostMessageResponse(
      env,
      `authorization:${PROVIDER}:error:` +
        JSON.stringify({ error: 'token_exchange_failed', provider: PROVIDER })
    );
  }
 ...
 */
function adminPostMessageResponse(env, message, options = {}) {
